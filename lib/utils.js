			...
			filter(context, filter.functions, [], err => {
				if (err) {
					if (err.constructor !== Error) 
						throw new Error(`${err.constructor.name}-error: ${err}`)
...
module.exports.includes = (a, b) => {
	return isArray(a) ? a.find(item => match(b, item)) !== undefined : a !== null && b in a;
}

//
// format value javascript style, tab used for function identation
//
const formatArray = Number(process.version.split('.')[0].substring(1)) < 12; 
// node v12.0.0 changes format(), ref https://nodejs.org/tr/blog/uncategorized/10-lts-to-12-lts/, see #23162
const jsformat = module.exports.jsformat = (value, tab) => { tab = tab || 0;
	if (value === undefined) return 'undefined';
	if (value === null) return 'null'; 
	switch (value.constructor) {
	case String  : return format("'%s'", value);
	...
	case Array   : return formatArray ? format("[%s ]", value.map(e => { return " " + jsformat(e, tab); })) : format('%s', value);
	case Object  : // same as default
	default      : return format("{%s\n%s}", Object.keys(value).map(key => { return format("\n%s%s: %s", Array(tab + 2).join('\t'), format(key.match(/^[_a-z][_a-z0-9]*$/i) ? "%s" : "'%s'", key), jsformat(value[key], tab + 1)); }), Array(tab + 1).join('\t'));
	}
}
...
const callAuxillarate = module.exports.callAuxillarate = (context, func, next) => {
	callFunction(context, func, 'auxillarating', next);
}
...
const mustBeType = module.exports.mustBeType = (context, source, key, type, next) => {
	next(new Error(`${context.fullpath()} must be type ${type.name} (not ${typeName(source[key])})`));
}
...
const mapValue = module.exports.mapValue = (context, value, map, next) => {
	if (likeObject(map)) {
		if (!isType(value, String)) return next(format(sourceMustBeOfType, context.fullpath(), 'String', typeName(value)));
		if ((value = map[value]) === undefined) return next(format(sourceMustBeIncluded, context.fullpath(), jsformat(Object.keys(map))));
	} else if (likeArray(map)) {
		if (!isType(value, Number)) return next(format(sourceMustBeOfType, context.fullpath(), 'Number', typeName(value)));
		if ((value = map[value]) === undefined) return next(format(sourceMustBeInRange, context.fullpath(), jsformat([0, map.length - 1])));
	} else {
...
module.exports.toArray = (context, target, key, next) => {
	...
	async.mapSeries(Object.entries(target), (item, next) => { next = defer(next); 
		if (item[1][key] !== undefined) return next(new Error(`${context.fullpath()}.${entry} already has object key: ${key} (${item[1][key]})`));
// TODO next assignment may fail, should be tested		
		item[1][key] = item[0];
		next(null, item[1]);
	}, next);
}
