...
module.exports.includes = (a, b) => {
	return isArray(a) ? a.find(item => match(b, item)) !== undefined : 
		   isObject(a) ? b in a : 
			          [a].find(item => match(b, item)) !== undefined;
}
...
//
// wrap called functions next-parameters to avoid zero or multiple callbacks
//
function exactlyOnce(context, text, next) {
	let timer;
	if (context.scope.timeout) timer = setTimeout(() => {
		throw new Error(`next() timed out after ${context.scope.timeout/1000} seconds in context: ${text}`);
	}, context.scope.timeout);
    const f = function (...args) {
    	if (timer) clearTimeout(timer);
        if (next === null) throw new Error(`next() was already called in context: ${text}`);
        const next2 = next;
        next = null;
...
//
//
const checkReturnType = module.exports.checkReturnType = (context, $modifier, types, next) => {
	return (err, value) => { if (err) return next(err);
		if (types.constructor === Array ? !isAnyType(value, types) : !isType(value, types)) return next(new Error(`${context.fullpath()}.${$modifier} function must return a value of type ${jsformat(types)}`));
		next(err, value);
	}
}
...
const callFunction = module.exports.callFunction = (context, func, text, next) => {
	try {
		func(context, context.source[context.key], exactlyOnce(context, `${text} ${context.fullpath()}`, (err, ...args) => {
			if (err) {
		...
		next(new Error(`throws error (${err.message}) while ${text}`));
	}
}
