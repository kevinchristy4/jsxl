'use strict';
// jsxl-filters compiled with jsxl-filters on Tue Jun 29 2021 18:42:49 GMT+0530 (India Standard Time)
...
Object.assign(filter, { 'Type1{}':
	function(context, functions, renamed, next) { // v1, key: Type1{}, timestamp: 1624972369183
...
filter['Type1{}'].timestamp = 1624972369183;
filter['Type1{}'].compiler  = 'v1';

Object.assign(filter, { 'Shell1{}':
	function(context, functions, renamed, next) { // v1, key: Shell1{}, timestamp: 1624972369176
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
...
filter['Shell1{}'].timestamp = 1624972369176;
filter['Shell1{}'].compiler  = 'v1';

Object.assign(filter, { 'Type1{$optional:true}':
	function(context, functions, renamed, next) { // v1, key: Type1{$optional:true}, timestamp: 1624972369179
		const source = context.source, target = context.target, key = context.key;
		async.series([ // any all
			next => (next => { // any option 
				async.waterfall([
...
filter['Type1{$optional:true}'].timestamp = 1624972369179;
filter['Type1{$optional:true}'].compiler  = 'v1';

Object.assign(filter, { 'Object1{}':
	function(context, functions, renamed, next) { // v1, key: Object1{}, timestamp: 1624972369211
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
...
filter['Object1{}'].timestamp = 1624972369211;
filter['Object1{}'].compiler  = 'v1';

Object.assign(filter, { 'Array1{}':
	function(context, functions, renamed, next) { // v1, key: Array1{}, timestamp: 1624972369211
		const source = context.source, target = context.target, key = context.key;
		async.series([ // functions
			next => { // filter
...
filter['Array1{}'].timestamp = 1624972369211;
filter['Array1{}'].compiler  = 'v1';

Object.assign(filter, { 'type1':
	function(context, functions, renamed, next) { // v1, key: type1, timestamp: 1624972369172
		const source = context.source, target = context.target, key = context.key;
		utils.intrinsic.filter['Type1{}'](context, utils.intrinsic.filter['Type1{}'].functions, renamed, next);
	}
});
filter['type1'].functions = [];
filter['type1'].timestamp = 1624972369172;
filter['type1'].compiler  = 'v1';
...
Object.assign(filter, { 'Type2{}':
	function(context, functions, renamed, next) { // v2, key: Type2{}, timestamp: 1624972369316
		const source = context.source, target = context.target, key = context.key;
		async.series([ // functions
			next => { // filter
...
filter['Type2{}'].timestamp = 1624972369316;
filter['Type2{}'].compiler  = 'v2';

Object.assign(filter, { 'Shell2{}':
	function(context, functions, renamed, next) { // v2, key: Shell2{}, timestamp: 1624972369319
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
...
filter['Shell2{}'].timestamp = 1624972369319;
filter['Shell2{}'].compiler  = 'v2';

Object.assign(filter, { 'Type2{$optional:true}':
	function(context, functions, renamed, next) { // v2, key: Type2{$optional:true}, timestamp: 1624972369319
		const source = context.source, target = context.target, key = context.key;
		async.series([ // functions
			next => { // filter
				utils.callFilter(context, functions[13], (err, keep) => { if (err) return next(err);
...
filter['Type2{$optional:true}'].timestamp = 1624972369319;
filter['Type2{$optional:true}'].compiler  = 'v2';

Object.assign(filter, { 'Shell2{$optional:true}':
	function(context, functions, renamed, next) { // v2, key: Shell2{$optional:true}, timestamp: 1624972369321
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
...
filter['Shell2{$optional:true}'].timestamp = 1624972369321;
...
module.exports.timestamp = 1624972369362;
