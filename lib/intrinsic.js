'use strict';
// jsxl-filters compiled with jsxl-filters on Fri Jul 09 2021 02:13:40 GMT+0530 (India Standard Time)
...
Object.assign(filter, { 'Type1{}':
	function(context, functions, renamed, next) { // v1, key: Type1{}, timestamp: 1625777020570
...
filter['Type1{}'].timestamp = 1625777020570;
filter['Type1{}'].compiler  = 'v1';

Object.assign(filter, { 'Shell1{}':
	function(context, functions, renamed, next) { // v1, key: Shell1{}, timestamp: 1625777020563
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
...
filter['Shell1{}'].timestamp = 1625777020563;
filter['Shell1{}'].compiler  = 'v1';

Object.assign(filter, { 'Type1{$optional:true}':
	function(context, functions, renamed, next) { // v1, key: Type1{$optional:true}, timestamp: 1625777020566
		const source = context.source, target = context.target, key = context.key;
		async.series([ // any all
			next => (next => { // any option 
				async.waterfall([
...
filter['Type1{$optional:true}'].timestamp = 1625777020566;
filter['Type1{$optional:true}'].compiler  = 'v1';

Object.assign(filter, { 'Object1{}':
	function(context, functions, renamed, next) { // v1, key: Object1{}, timestamp: 1625777020599
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
...
filter['Object1{}'].timestamp = 1625777020599;
filter['Object1{}'].compiler  = 'v1';

Object.assign(filter, { 'Array1{}':
	function(context, functions, renamed, next) { // v1, key: Array1{}, timestamp: 1625777020600
		const source = context.source, target = context.target, key = context.key;
		async.series([ // functions
			next => { // filter
...
filter['Array1{}'].timestamp = 1625777020600;
filter['Array1{}'].compiler  = 'v1';

Object.assign(filter, { 'type1':
	function(context, functions, renamed, next) { // v1, key: type1, timestamp: 1625777020559
		const source = context.source, target = context.target, key = context.key;
		utils.intrinsic.filter['Type1{}'](context, utils.intrinsic.filter['Type1{}'].functions, renamed, next);
	}
});
filter['type1'].functions = [];
filter['type1'].timestamp = 1625777020559;
filter['type1'].compiler  = 'v1';
...
Object.assign(filter, { 'Type2{}':
	function(context, functions, renamed, next) { // v2, key: Type2{}, timestamp: 1625777020709
		const source = context.source, target = context.target, key = context.key;
		async.series([ // functions
			next => { // filter
		...
		(context, filter, next) => next(null, utils.isType(filter, String)),(context, filter, next) => compiler2.compileString(context, next),(context, filter, next) => {
					...
					if (utils.isObject(filter) && (utils.overlap(Object.keys(filter), utils.modifiers) && context.key != '$escape')) { 
						...
						(next => {
							...
							if (context.scope.modifiers.$rename && context.scope.inside != 'object') return next(new Error(`.$rename cannot rename if not inside object`))
				...
				},(context, filter, next) => compiler2.compileShell(context, next),(context, filter, next) => next(null, utils.isFunction(filter)),(context, filter, next) => { 
					utils.isFilter(filter) ?
				...
				},(context, filter, next) => next(null, utils.isObject(filter)),(context, filter, next) => compiler2.compileObject(context, next),(context, filter, next) => { 
...
filter['Type2{}'].timestamp = 1625777020709;
filter['Type2{}'].compiler  = 'v2';

Object.assign(filter, { 'Shell2{}':
	function(context, functions, renamed, next) { // v2, key: Shell2{}, timestamp: 1625777020711
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
...
filter['Shell2{}'].timestamp = 1625777020711;
filter['Shell2{}'].compiler  = 'v2';

Object.assign(filter, { 'Type2{$optional:true}':
	function(context, functions, renamed, next) { // v2, key: Type2{$optional:true}, timestamp: 1625777020711
		const source = context.source, target = context.target, key = context.key;
		async.series([ // functions
			next => { // filter
				utils.callFilter(context, functions[13], (err, keep) => { if (err) return next(err);
...
filter['Type2{$optional:true}'].timestamp = 1625777020711;
filter['Type2{$optional:true}'].compiler  = 'v2';

Object.assign(filter, { 'Shell2{$optional:true}':
	function(context, functions, renamed, next) { // v2, key: Shell2{$optional:true}, timestamp: 1625777020712
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
...
filter['Shell2{$optional:true}'].timestamp = 1625777020712;
filter['Shell2{$optional:true}'].compiler  = 'v2';

Object.assign(filter, { 'type2':
	function(context, functions, renamed, next) { // v2, key: type2, timestamp: 1625777020708
		const source = context.source, target = context.target, key = context.key;
		utils.intrinsic.filter['Type2{}'](context, utils.intrinsic.filter['Type2{}'].functions, renamed, next);
	}
});
filter['type2'].functions = [];
filter['type2'].timestamp = 1625777020708;
filter['type2'].compiler  = 'v2';

module.exports.timestamp = 1625777020748;
