'use strict';
// jsxl-filters compiled with jsxl-filters on Mon Jun 14 2021 18:04:25 GMT+0530 (India Standard Time)
...
Object.assign(filter, { 'Type1{}':
	function(context, functions, renamed, next) { // v1, key: Type1{}, timestamp: 1623674065200
...
filter['Type1{}'].timestamp = 1623674065200;
filter['Type1{}'].compiler  = 'v1';

Object.assign(filter, { 'Shell1{}':
	function(context, functions, renamed, next) { // v1, key: Shell1{}, timestamp: 1623674065190
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
...
filter['Shell1{}'].timestamp = 1623674065190;
filter['Shell1{}'].compiler  = 'v1';

Object.assign(filter, { 'Type1{$optional:true}':
	function(context, functions, renamed, next) { // v1, key: Type1{$optional:true}, timestamp: 1623674065194
		const source = context.source, target = context.target, key = context.key;
		async.series([ // any all
			next => (next => { // any option 
				async.waterfall([
...
filter['Type1{$optional:true}'].timestamp = 1623674065194;
filter['Type1{$optional:true}'].compiler  = 'v1';

Object.assign(filter, { 'Object1{}':
	function(context, functions, renamed, next) { // v1, key: Object1{}, timestamp: 1623674065239
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
...
filter['Object1{}'].timestamp = 1623674065239;
filter['Object1{}'].compiler  = 'v1';

Object.assign(filter, { 'Array1{}':
	function(context, functions, renamed, next) { // v1, key: Array1{}, timestamp: 1623674065241
		const source = context.source, target = context.target, key = context.key;
		async.series([ // functions
			next => { // filter
...
filter['Array1{}'].timestamp = 1623674065241;
filter['Array1{}'].compiler  = 'v1';

Object.assign(filter, { 'type1':
	function(context, functions, renamed, next) { // v1, key: type1, timestamp: 1623674065182
		const source = context.source, target = context.target, key = context.key;
		utils.intrinsic.filter['Type1{}'](context, utils.intrinsic.filter['Type1{}'].functions, renamed, next);
	}
});
filter['type1'].functions = [];
filter['type1'].timestamp = 1623674065182;
filter['type1'].compiler  = 'v1';

Object.assign(filter, { 'shell1':
	function(context, functions, renamed, next) { // v1, key: shell1, timestamp: 1623674065189
		const source = context.source, target = context.target, key = context.key;
		utils.intrinsic.filter['Shell1{}'](context, utils.intrinsic.filter['Shell1{}'].functions, renamed, next);
	}
});
filter['shell1'].functions = [];
filter['shell1'].timestamp = 1623674065189;
filter['shell1'].compiler  = 'v1';

Object.assign(filter, { 'object1':
	function(context, functions, renamed, next) { // v1, key: object1, timestamp: 1623674065238
		const source = context.source, target = context.target, key = context.key;
		utils.intrinsic.filter['Object1{}'](context, utils.intrinsic.filter['Object1{}'].functions, renamed, next);
	}
});
filter['object1'].functions = [];
filter['object1'].timestamp = 1623674065238;
filter['object1'].compiler  = 'v1';

Object.assign(filter, { 'array1':
	function(context, functions, renamed, next) { // v1, key: array1, timestamp: 1623674065240
		const source = context.source, target = context.target, key = context.key;
		utils.intrinsic.filter['Array1{}'](context, utils.intrinsic.filter['Array1{}'].functions, renamed, next);
	}
});
filter['array1'].functions = [];
filter['array1'].timestamp = 1623674065240;
filter['array1'].compiler  = 'v1';

Object.assign(filter, { 'Type2{}':
	function(context, functions, renamed, next) { // v2, key: Type2{}, timestamp: 1623674065429
		const source = context.source, target = context.target, key = context.key;
		async.series([ // functions
			next => { // filter
...
filter['Type2{}'].timestamp = 1623674065429;
filter['Type2{}'].compiler  = 'v2';

Object.assign(filter, { 'Shell2{}':
	function(context, functions, renamed, next) { // v2, key: Shell2{}, timestamp: 1623674065433
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
...
filter['Shell2{}'].timestamp = 1623674065433;
filter['Shell2{}'].compiler  = 'v2';

Object.assign(filter, { 'Type2{$optional:true}':
	function(context, functions, renamed, next) { // v2, key: Type2{$optional:true}, timestamp: 1623674065434
		const source = context.source, target = context.target, key = context.key;
		async.series([ // functions
			next => { // filter
				utils.callFilter(context, functions[13], (err, keep) => { if (err) return next(err);
...
filter['Type2{$optional:true}'].timestamp = 1623674065434;
filter['Type2{$optional:true}'].compiler  = 'v2';

Object.assign(filter, { 'Shell2{$optional:true}':
	function(context, functions, renamed, next) { // v2, key: Shell2{$optional:true}, timestamp: 1623674065435
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
...
filter['Shell2{$optional:true}'].timestamp = 1623674065435;
filter['Shell2{$optional:true}'].compiler  = 'v2';

Object.assign(filter, { 'type2':
	function(context, functions, renamed, next) { // v2, key: type2, timestamp: 1623674065428
		const source = context.source, target = context.target, key = context.key;
		utils.intrinsic.filter['Type2{}'](context, utils.intrinsic.filter['Type2{}'].functions, renamed, next);
	}
});
filter['type2'].functions = [];
filter['type2'].timestamp = 1623674065428;
filter['type2'].compiler  = 'v2';

module.exports.timestamp = 1623674065490;
