'use strict';
// jsxl-filters compiled with jsxl-filters on Mon Jun 07 2021 02:49:00 GMT+0530 (India Standard Time)
...
Object.assign(filter, { 'Type1{}':
	function(context, functions, renamed, next) { // v1, key: Type1{}, timestamp: 1623014340707
...
filter['Type1{}'].timestamp = 1623014340707;
filter['Type1{}'].compiler  = 'v1';

Object.assign(filter, { 'Shell1{}':
	function(context, functions, renamed, next) { // v1, key: Shell1{}, timestamp: 1623014340700
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
...
filter['Shell1{}'].timestamp = 1623014340700;
filter['Shell1{}'].compiler  = 'v1';

Object.assign(filter, { 'Type1{$optional:true}':
	function(context, functions, renamed, next) { // v1, key: Type1{$optional:true}, timestamp: 1623014340703
		const source = context.source, target = context.target, key = context.key;
		async.series([ // any all
			next => (next => { // any option 
				async.waterfall([
...
filter['Type1{$optional:true}'].timestamp = 1623014340703;
filter['Type1{$optional:true}'].compiler  = 'v1';

Object.assign(filter, { 'Object1{}':
	function(context, functions, renamed, next) { // v1, key: Object1{}, timestamp: 1623014340738
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
...
filter['Object1{}'].timestamp = 1623014340738;
filter['Object1{}'].compiler  = 'v1';

Object.assign(filter, { 'Array1{}':
	function(context, functions, renamed, next) { // v1, key: Array1{}, timestamp: 1623014340739
		const source = context.source, target = context.target, key = context.key;
		async.series([ // functions
			next => { // filter
...
filter['Array1{}'].timestamp = 1623014340739;
filter['Array1{}'].compiler  = 'v1';

Object.assign(filter, { 'type1':
	function(context, functions, renamed, next) { // v1, key: type1, timestamp: 1623014340696
		const source = context.source, target = context.target, key = context.key;
		utils.intrinsic.filter['Type1{}'](context, utils.intrinsic.filter['Type1{}'].functions, renamed, next);
	}
});
filter['type1'].functions = [];
filter['type1'].timestamp = 1623014340696;
filter['type1'].compiler  = 'v1';

Object.assign(filter, { 'shell1':
	function(context, functions, renamed, next) { // v1, key: shell1, timestamp: 1623014340699
		const source = context.source, target = context.target, key = context.key;
		utils.intrinsic.filter['Shell1{}'](context, utils.intrinsic.filter['Shell1{}'].functions, renamed, next);
	}
});
filter['shell1'].functions = [];
filter['shell1'].timestamp = 1623014340699;
filter['shell1'].compiler  = 'v1';
...
Object.assign(filter, { 'Type2{}':
	function(context, functions, renamed, next) { // v2, key: Type2{}, timestamp: 1623014340845
		const source = context.source, target = context.target, key = context.key;
		async.series([ // functions
			next => { // filter
...
filter['Type2{}'].timestamp = 1623014340845;
filter['Type2{}'].compiler  = 'v2';

Object.assign(filter, { 'Shell2{}':
	function(context, functions, renamed, next) { // v2, key: Shell2{}, timestamp: 1623014340846
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
...
filter['Shell2{}'].timestamp = 1623014340846;
filter['Shell2{}'].compiler  = 'v2';

Object.assign(filter, { 'Type2{$optional:true}':
	function(context, functions, renamed, next) { // v2, key: Type2{$optional:true}, timestamp: 1623014340847
		const source = context.source, target = context.target, key = context.key;
		async.series([ // functions
			next => { // filter
				utils.callFilter(context, functions[13], (err, keep) => { if (err) return next(err);
...
filter['Type2{$optional:true}'].timestamp = 1623014340847;
filter['Type2{$optional:true}'].compiler  = 'v2';

Object.assign(filter, { 'Shell2{$optional:true}':
	function(context, functions, renamed, next) { // v2, key: Shell2{$optional:true}, timestamp: 1623014340848
		const source = context.source, target = context.target, key = context.key;
		async.waterfall([
			next => {
				if (source[key] === undefined) return utils.mustBeProvided(context, next);
...
filter['Shell2{$optional:true}'].timestamp = 1623014340848;
filter['Shell2{$optional:true}'].compiler  = 'v2';

Object.assign(filter, { 'type2':
	function(context, functions, renamed, next) { // v2, key: type2, timestamp: 1623014340844
		const source = context.source, target = context.target, key = context.key;
		utils.intrinsic.filter['Type2{}'](context, utils.intrinsic.filter['Type2{}'].functions, renamed, next);
	}
});
filter['type2'].functions = [];
filter['type2'].timestamp = 1623014340844;
filter['type2'].compiler  = 'v2';

module.exports.timestamp = 1623014340881;
