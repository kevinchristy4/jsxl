...
//module.exports.assert2 = (modifier, functions, assertion, message) => 
//[
//	utils.isFunction(modifier) ?
//		`utils.callModify(context, functions[${utils.pushReturnLength(functions, modifier)}], next);` :
//		`next(null, ${utils.jsformat(modifier)});`,
//	`}, (modifier, next) => {`,
////	message ?
////		`if (${assertion}) return utils.errorMessage(context, ${JSON.stringify(message)}, modifier, next);`:
//		`if (err = (${assertion})) return utils.errorMessage(context, err, modifier, next);`
//];
...
[
	`if (!utils.isType(source[key], '${filter.name}')) return utils.mustBeType(context, source, key, '${filter.name}', next);` // no need to JSON stringify, these are always identifiers
];
