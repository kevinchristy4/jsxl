...
[
	...(utils.isFunction(scope) ? [
			`utils.callScope(context, functions[${utils.pushReturnLength(functions, scope)}], utils.checkReturnType(context, '$scope', ${utils.jsformat(globals.scopeType)}, (err, $scope) => { if (err) return next(err);`,
...
[
	utils.isFunction(map) ?    
		`utils.callMap(context, functions[${utils.pushReturnLength(functions, map)}], utils.checkReturnType(context, '$map', ${utils.jsformat(globals.collectionTypes)}, next));` :
...
module.exports.assert = (modifiers, $modifier, types, functions, assertion, error, message) => 
[
	utils.isFunction(modifiers[$modifier]) ?
	types ?
		`utils.callModify(context, functions[${utils.pushReturnLength(functions, modifiers[$modifier])}], utils.checkReturnType(context, '${$modifier}', ${utils.jsformat(types)}, next));` :
		`utils.callModify(context, functions[${utils.pushReturnLength(functions, modifiers[$modifier])}], next);` :
		`next(null, ${utils.jsformat(modifiers[$modifier])});`,
	`}, (modifier, next) => {`,
	message ?
...
const renamed = (rename, functions) => { 
	...
	[
		utils.isFunction(rename) ?    
			`utils.callRename(context, functions[${utils.pushReturnLength(functions, rename)}], utils.checkReturnType(context, '$rename', ${utils.jsformat(globals.nameType)}, next));` :
...
[
	`if (!utils.isTypeName(source[key], '${filter.name}')) return utils.mustBeType(context, source, key, '${filter.name}', next);` // no need to JSON stringify, these are always identifiers
];
