...
const compileAssert = module.exports.compileAssert = (context, script, next) => { // next(err, scriptNext)
	const compileModifier = (script, modifier, assertion, error, message, next) => {
		if (modifier !== undefined)
			script.push(
				...coder.assert(modifier, context.scope.functions, assertion, error, message)
			);
		next(null);
	}
//	const compileModifier2 = (script, modifier, assertion, message, next) => {
//		if (modifier !== undefined)
//			script.push(
//				...coder.assert2(modifier, context.scope.functions, assertion, message)
//			);
//		next(null);
//	}
	const modifiers = context.scope.modifiers;
	...
	async.series([
		next => compileModifier(script, modifiers['$length'		], ` utils.length(source[key]) == modifier`, 		'must have exact length',		   				modifiers.$message, next),
		next => compileModifier(script, modifiers['$minlen'		], ` utils.length(source[key]) >= modifier`, 		'must have minimum length', 					modifiers.$message, next),
		next => compileModifier(script, modifiers['$maxlen'		], ` utils.length(source[key]) <= modifier`, 		'must have maximum length',		 				modifiers.$message, next),
		
		next => compileModifier(script, modifiers['$name'		], ` utils.name(source[key])        == modifier`,	'must have name',            					modifiers.$message, next),
		next => compileModifier(script, modifiers['$constructor'], ` utils.constructor(source[key]) == modifier`, 	'must have constructor name', 					modifiers.$message, next),

		next => compileModifier(script, modifiers['$lt'			], `source[key] <   modifier`, 						'must be less than',	 						modifiers.$message, next),
		next => compileModifier(script, modifiers['$lte'		], `source[key] <=  modifier`, 						'must be less than or equal to',				modifiers.$message, next),
		next => compileModifier(script, modifiers['$eq'			], `source[key] === modifier`, 						'must be equal to',		 						modifiers.$message, next),
		next => compileModifier(script, modifiers['$ne'			], `source[key] !== modifier`, 						'must be different from', 						modifiers.$message, next),
		next => compileModifier(script, modifiers['$gte'		], `source[key] >=  modifier`, 						'must be greater than or equal to', 			modifiers.$message, next),
		next => compileModifier(script, modifiers['$gt'			], `source[key] >   modifier`,						'must be greater than', 						modifiers.$message, next), 
		
		next => compileModifier(script, modifiers['$match'		], ` utils.match(source[key], modifier)`,			'must match', 									modifiers.$message, next),

		next => compileModifier(script, modifiers['$in'			], ` utils.includes(modifier, source[key])`, 		'must be included in',							modifiers.$message, next),
		next => compileModifier(script, modifiers['$nin'		], `!utils.includes(modifier, source[key])`, 		'must be excluded from',	 					modifiers.$message, next),
		next => compileModifier(script, modifiers['$inc'		], ` utils.includes(source[key], modifier)`, 		'must include',									modifiers.$message, next),
		next => compileModifier(script, modifiers['$ninc'		], `!utils.includes(source[key], modifier)`, 		'must exclude',									modifiers.$message, next),
		
//		next => compileModifier2(script, modifiers['$length'		], `(msg => utils.length(source[key]) == modifier ? false : msg)('must have exact length')`,			modifiers.$message, next),
//		next => compileModifier2(script, modifiers['$minlen'		], `(msg => utils.length(source[key]) >= modifier ? false : msg)('must have minimum length')`, 			modifiers.$message, next),
//		next => compileModifier2(script, modifiers['$maxlen'		], `(msg => utils.length(source[key]) <= modifier ? false : msg)('must have maximum length')`,			modifiers.$message, next),
//
//		next => compileModifier2(script, modifiers['$name'			], `(msg => utils.name(source[key])        == modifier ? false : msg)('must have name')`, 				modifiers.$message, next),
//		next => compileModifier2(script, modifiers['$constructor'	], `(msg => utils.constructor(source[key]) == modifier ? false : msg)('must have constructor name')`,	modifiers.$message, next),
//
//		next => compileModifier2(script, modifiers['$match'			], ` utils.match(source[key], modifier) ? false : ${JSON.stringify(modifiers.$message)} || 'must match'`,			 				modifiers.$message, next),
//		next => compileModifier2(script, modifiers['$lt'			], `(msg => source[key] <  modifier ? false : msg)('must be less than')`, 							modifiers.$message, next),
//		next => compileModifier2(script, modifiers['$lte'			], `(msg => source[key] <= modifier ? false : msg)('must be less than or equal to')`,				modifiers.$message, next),
//		next => compileModifier2(script, modifiers['$eq'			], `(msg => source[key] == modifier ? false : msg)('must be equal to')`, 							modifiers.$message, next),
//		next => compileModifier2(script, modifiers['$neq'			], `(msg => source[key] != modifier ? false : msg)('must be different from')`,						modifiers.$message, next),
//		next => compileModifier2(script, modifiers['$gte'			], `(msg => source[key] >= modifier ? false : msg)('must be greater than or equal to')`, 			modifiers.$message, next),
//		next => compileModifier2(script, modifiers['$gt'			], `(msg => source[key] >  modifier ? false : msg)('must be greater than')`, 						modifiers.$message, next),

//		next => compileModifier(script, modifiers['$validate'	], `validate(source[key], modifier)`, undefined, modifiers.$message, next),

		//		next => {
//			async.mapSeries(X, (mod, next) => {
//				compileModifier(script, modifiers[mod.name			], `source[key] >   modifier`, `utils.sourceMustBeGT`, modifiers.$message, next)
//			}, next);
//		}
