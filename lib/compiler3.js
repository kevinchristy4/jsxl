...
module.exports.compile = (filter, options, compileType, vX, next) => {
	...
	// preludes - used for declaring referenced functions
...
const compileSource = module.exports.compileSource = (context, script, next) => { // next(err, scriptMore)
	...
	if (modifiers) {
		...
		if (rid == 1 && modifiers.$optional === false) return next(new Error(`${context.fullpath()} holds any of $remove, $insert, or $default modifiers but is explicitly non-optional`));
	}
...
// test for source.key !== null
	...
	// if args is set, assign to context, do not return 
	if (modifiers && '$args' in modifiers)
		scriptMore.push(
			...coder.sourceArgs(modifiers.$args, context.scope.functions)
		);
	// if insert, do return
...
// TODO what if any key is non-identifer, could we merge with 2 options below?
		...
		(scriptMore, scriptKeys, scriptRest, next) => {
			// fail renaming if context is not object 
// TODO test if this still works
// TODO what to do if $-attribue is renamed			
			if (modifiers.$rename && (!context.context || context.context.target.constructor !== Object)) return next(new Error(`${context.fullpath()} cannot rename`));
