...
const compileAssert = module.exports.compileAssert = (context, script, next) => { // next(err, scriptNext)
	...
	async.series([
		...
		next => compileModifier(script, modifiers['$in'			], ` utils.includes(modifier, source[key])`, 		'must be included in array or object',			modifiers.$message, next),
		...
		next => compileModifier(script, modifiers['$inc'		], ` utils.includes(source[key], modifier)`, 		'must be of type array or object and include',	modifiers.$message, next),
