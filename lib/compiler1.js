...
//	const filter = context.target[context.rename];
	...
	async.waterfall([
		...
		next => {
			if (context.scope.modifiers.$rename && context.scope.inside == 'array') return next(new Error(`${new Context(context, '$rename').fullpath()} cannot rename when inside array`))
