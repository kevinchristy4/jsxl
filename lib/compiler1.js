...
//	const filter = context.target[context.rename];
	...
	async.waterfall([
		...
		next => {
			if (context.scope.modifiers.$rename && context.scope.inside != 'object') {
				console.log(3344, context);
				return next(new Error(`${new Context(context, '$rename').fullpath()} cannot rename if not inside object`))
			}
