...
if (this.key === undefined) console.log(3434, this);	
	...
	case Array:  return this.path + `['${this.key.join("|")}']`; 
	case String: return this.path + (this.key.match(/^[\$a-zA-Z_][\.\:\$a-zA-Z_0-9]*$/) ? (this.path ? '.' : '') + `${this.key}` : `['${this.key}']`);
...
//
Context.prototype.shift = function(x) {
	if (utils.likeArray(this.source)) { 
		this.source[this.key] = this.target[this.key];
//if (this.target === this.source) throw 8765;		
// TODO we should avoid ever having target === source
